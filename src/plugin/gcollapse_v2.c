...
int sf_collapse (struct StataInfo *st_info)
{
    ...
    if ( st_info->benchmark ) sf_running_timer (&timer, "\tPlugin step 6: Copied group variables back to stata");
...
int sf_wrapper ()
{
    ...
    int byvars_lens[kvars_by],
        byvars_mins[kvars_by],
        byvars_maxs[kvars_by];
    ...
    size_t kvars_by_str = 0;
    for (i = 0; i < kvars_by; i++) {
        kvars_by_str += (byvars_lens[i] > 0);
    }
    size_t kvars_by_num = kvars_by - kvars_by_str;

    // If only integers, check worst case of the bijection would not overflow
    int integers_ok;
    int byvars_minlen = mf_min_signed(byvars_lens, kvars_by);
    int byvars_maxlen = mf_max_signed(byvars_lens, kvars_by);
    if ( byvars_maxlen < 0 ) {
        if (kvars_by > 1) {
            integers_ok = 1;
            size_t worst = byvars_maxs[0] - byvars_mins[0] + 1;
            size_t range = byvars_maxs[1] - byvars_mins[1] + (1 < (kvars_by - 1));
            for (k = 1; k < kvars_by; k++) {
                if ( worst > (ULONG_MAX / range)  ) {
                    ...
                    worst *= range;
                    range  = byvars_maxs[k] - byvars_mins[k] + (k < (kvars_by - 1));
                }
            }
        }
        else {
    ...
    // size_t start_target_vars = start_collapse_vars;
    size_t start_target_vars = start_collapse_vars + kvars_source;
    int pos_targets[kvars_targets];
    double pos_targets_double[kvars_targets];

    size_t strlen = byvars_maxlen > 0? byvars_maxlen + 1: 1;
    size_t start_str_byvars = start_target_vars + kvars_targets;
    int pos_str_byvars[kvars_by_str];
    double pos_str_byvars_double[kvars_by_str];
    char s[strlen];

    int pos_num_byvars[kvars_by_num];
    double pos_num_byvars_double[kvars_by_num];
    ...
    if ( kvars_by_str > 0 ) {
        if ( (rc = sf_get_vector("__gtools_strpos", pos_str_byvars_double)) ) return(rc);
        for (k = 0; k < kvars_by_str; k++)
            pos_str_byvars[k] = (int) pos_str_byvars_double[k];
    }

    if ( kvars_by_num > 0 ) {
        if ( (rc = sf_get_vector("__gtools_numpos", pos_num_byvars_double)) ) return(rc);
        for (k = 0; k < kvars_by_num; k++)
            pos_num_byvars[k] = (int) pos_num_byvars_double[k];
    }
    ...
    if ( integers_ok ) {
        ...
        if (kvars_by > 1) {
            ...
            if ( (rc = sf_get_varlist_bijection (ghash1, 1, kvars_by, in1, in2, byvars_mins, byvars_maxs)) ) return(rc);
        }
        else {
        ...
        if (kvars_by > 1) {
            if ( verbose ) {
                if ( byvars_maxlen > 0 ) {
                    ...
                    sf_printf("Using 128-bit hash to index %d numeric-only by variables", kvars_by);
                }
            }
            if ( (rc = sf_get_varlist_hash (ghash1, ghash2, 1, kvars_by, in1, in2, byvars_lens)) ) return(rc);
        }
        else {
    ...
    size_t nmfreq[kvars_source], nonmiss[kvars_source], firstmiss[kvars_source], lastmiss[kvars_source];
    double *bynum   = calloc(kvars_by_num * J, sizeof *bynum);
    short  *bymiss  = calloc(kvars_by_num * J, sizeof *bymiss);
    double *output  = calloc(kvars_targets * J, sizeof *output);
    short  *outmiss = calloc(kvars_targets * J, sizeof *outmiss);
    double *buffer  = calloc(kvars_source * nj_max, sizeof *output);
    ...
    for (i = 0; i < kvars_by_num * J; i++)
        bymiss[i] = 0;

    for (i = 0; i < kvars_targets * J; i++)
        outmiss[i] = 0;

    for (k = 0; k < kvars_source; k++)
        nmfreq[k] = nonmiss[k] = firstmiss[k] = lastmiss[k] = 0;
     ...
     *  Testing  *
     *************/
    double *all_buffer    = calloc(kvars_source * N, sizeof *output);
    short  *all_firstmiss = calloc(kvars_source * J, sizeof *output);
    short  *all_lastmiss  = calloc(kvars_source * J, sizeof *output);
    size_t *all_nonmiss   = calloc(kvars_source * J, sizeof *output);
    ...
    for (j = 0; j < J * kvars_source; j++)
        all_firstmiss[j] = all_lastmiss[j] = all_nonmiss[j] = 0;

    // #pragma omp parallel private (i, k, offset_buffer, offset_source, offset_output, sel, start, end, nj) shared (verbose, output, outmiss, stat, all_firstmiss, all_lastmiss, offsets_buffer, all_nonmiss, kvars_targets, pos_targets, kvars_source, info, all_buffer, index)
    // {
    // }
    offset_buffer = offset_source = 0;
    for (j = 0; j < J; j++) {
        ...
        for (i = start; i < end; i++) {
            ...
            sel = i + in1;
            for (k = 0; k < kvars_source; k++) {
                if ( (rc = SF_vdata(k + start_collapse_vars, sel, &z)) ) return(rc);
                if ( SF_is_missing(z) ) {
                    if (i == start)   all_firstmiss[offset_source + k] = 1;
                    if (i == end - 1) all_lastmiss[offset_source + k]  = 1;
                }
        ...
        offsets_buffer[j] = offset_buffer;
        // parallel
        offset_buffer += nj * kvars_source;
        offset_source += kvars_source;
    }
    ...
    for (j = 0; j < J; j++)
        for (k = 0; k < kvars_source; k++)
            nmfreq[k] += all_nonmiss[j * kvars_source + k];
    // parallel

    // #pragma omp parallel private (k, offset_buffer, offset_source, offset_output, sel, start, end, nj) shared (verbose, output, outmiss, stat, all_firstmiss, all_lastmiss, offsets_buffer, all_nonmiss, kvars_targets, pos_targets, kvars_source, info, all_buffer)
    // {
        // parallel
        // nloops        = 0;
        // offset_buffer = 0;
        ...
        // offset_source = offset_output = 0;
        // single thread
        // #pragma omp for
        for (j = 0; j < J; j++) {
            // parallel
            // ++nloops;
            offset_output = j * kvars_targets;
            offset_source = j * kvars_source;
            offset_buffer = offsets_buffer[j];
            // parallel
            nj = info[j + 1] - info[j];
            for (k = 0; k < kvars_targets; k++) {
            ...
            // single thread
            // offset_buffer += (info[j + 1] - info[j]) * kvars_source;
            // for (k = 0; k < kvars_source; k++)
            //     nmfreq[k] += all_nonmiss[offset_source + k];
            //
            // offset_output += kvars_targets;
            // offset_source += kvars_source;
            // single thread
        }
        // #pragma omp critical
    ...
    for (j = 0; j < J; j++) {
        start = info[j];
        for (k = 0; k < kvars_by_str; k++) {
            if ( (rc = SF_sdata(pos_str_byvars[k], index[start] + in1, s)) ) return(rc);
            if ( (rc = SF_sstore(k + start_str_byvars, j + 1, s)) ) return(rc);
        }
        for (k = 0; k < kvars_by_num; k++) {
            if ( (rc = SF_vdata(pos_num_byvars[k], index[start] + in1, &z)) ) return(rc);
            if ( SF_is_missing(z) ) {
                bymiss[offset_bynum + k] = 1;
            }
            else {
    ...
    for (j = 0; j < J; j++) {
        ...
        for (i = start; i < end; i++) {
            for (k = 0; k < kvars_source; k++) {
                if ( (rc = SF_vdata(k + start_collapse_vars, index[i] + in1, &z)) ) return(rc);
                if ( SF_is_missing(z) ) {
        ...
        for (k = 0; k < kvars_targets; k++) {
            ...
            if ( mf_strcmp_wrapper (stat[k], "count") ) {
                ...
                output[offset_output + k] = buffer[start];
            }
            else if ( mf_strcmp_wrapper (stat[k], "last") | (mf_strcmp_wrapper (stat[k], "lastnm") ) ) {
                output[offset_output + k] = buffer[start + end - 1];
            }
            else if ( mf_strcmp_wrapper (stat[k], "sd") &  (end < 2) ) {
                outmiss[offset_output + k] = 1;
            }
                ...
                output[offset_output + k] = mf_switch_fun (stat[k], buffer, start, start + end);
            }
        }
        offset_output += kvars_targets;
    ...
    for (j = 0; j < J; j++) {
        for (k = 0; k < kvars_targets; k++) {
            ...
            if ( (rc = SF_vstore(k + start_target_vars, j + 1, outmiss[sel]? SV_missval: output[sel])) ) return (rc);
        }
        for (k = 0; k < kvars_by_str; k++) {
            if ( (rc = SF_sdata(k + start_str_byvars, j + 1, s)) ) return(rc);
            if ( (rc = SF_sstore(pos_str_byvars[k], j + 1, s)) ) return(rc);
        }
        for (k = 0; k < kvars_by_num; k++) {
            sel = offset_bynum + k;
            if ( (rc = SF_vstore(pos_num_byvars[k], j + 1, bymiss[sel]? SV_missval: bynum[sel])) ) return(rc);
        }
        offset_output += kvars_targets;
        offset_bynum += kvars_by_num;
    }
    if ( benchmark ) sf_running_timer (&timer, "\tPlugin step 6: Copied group variables back to stata");
