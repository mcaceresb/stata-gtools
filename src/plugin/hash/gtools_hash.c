    ...
    struct timespec stimer)
{
    ...
    if ( st_info->biject ) {
        ...
        if ( st_info->benchmark > 2 )
            sf_running_timespec (&stimer, "\t\tPlugin step 2.3: Bijected integers to natural numbers");
        ...
        if ( !sorted ) {
            ...
            if ( st_info->benchmark > 2 )
                sf_running_timespec (&stimer, "\t\tPlugin step 2.4: Sorted integer-only hash");
        }
        ...
        if ( st_info->benchmark > 2 )
            sf_running_timespec (&stimer, "\t\tPlugin step 2.3: Hashed variables (128-bit)");
