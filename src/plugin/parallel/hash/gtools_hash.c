    ...
    clock_t stimer)
{
    ...
    if ( st_info->biject ) {
        if ( (rc = mf_biject_varlist (h1, st_info)) ) goto exit;
        ...
        if ( kstr > 0 ) {
            for (i = 0; i < N; i++)
                spookyhash_128(st_info->st_charx + (i * rowbytes),
                               sizeof(char) * rowbytes, h1 + i, h3 + i);
        }
        else {
            for (i = 0; i < N; i++)
                spookyhash_128(st_info->st_numx + i * kvars,
                               sizeof(double) * kvars, h1 + i, h3 + i);
        }
...
bycopy:
    rowbytes = st_info->rowbytes + sizeof(size_t);
    if ( (level > 0) & (st_info->countonly == 0) ) {
        if ( kstr > 0 ) {
            ...
            for (j = 0; j < st_info->J; j++) {
                ...
                memcpy (st_info->st_by_charx + sel, &j, sizeof(size_t));
            }
    ...
    if ( (level > 0) & (st_info->countonly == 0) ) {
        ...
        if ( kstr > 0 ) {
            for (j = 0; j < st_info->J; j++) {
                st_info->ix[j] = *((size_t *) (st_info->st_by_charx + j * rowbytes + st_info->positions[kvars]));
            }
        }
        else {
            for (j = 0; j < st_info->J; j++) {
                st_info->ix[j] = (size_t) st_info->st_by_numx[j * (kvars + 1) + kvars];
            }
        }
    }
